...
npm run dev-debug                       # Dev server with Node inspector
...
# Docker
docker build -t big-agi .               # Build Docker image
...
**Testing**: The project has test files in `references/` (excluded from build). There is no active test runner configured in `package.json`. Test critical user flows manually.

**Installation & Deployment**: See `docs/installation.md` for setup, `docs/deploy-docker.md` for Docker, `docs/deploy-database.md` for database deployment.
...
For the BigAGI context-management system (two-agent compression, categories, file memory, rollback), the canonical product intent document is:
...
3. Actual code paths
4. Repository docs (`CLAUDE.md`, README) as engineering context only

**Important**: `CLAUDE.md` describes repository architecture and development context. It does NOT override the canonical product intent for the context-management system.

**Code references**: Use materials from `/references/` as code/prompt references.

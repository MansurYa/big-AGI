"""
Manual quality assessment for Agent 1 on medium-sized context (~65k tokens).
"""
...
def manual_quality_check_medium():
    """
    Run Agent 1 on ~65k token context (messages 1-2).
    """
    print("=== AGENT 1 QUALITY CHECK (MEDIUM: ~65k tokens) ===\n")
    ...
    print(f"Context: messages 1-2")
    print(f"Total tokens: {actual_tokens}")
    print(f"Total lines: {len(context.split(chr(10)))}\n")

    # Run Agent 1
    agent1 = Agent1Selector()
    need_to_free = int(actual_tokens * 0.3)  # 30% of context

    print(f"Requesting to free: {need_to_free} tokens (~30% of context)\n")

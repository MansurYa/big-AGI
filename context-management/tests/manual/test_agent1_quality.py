"""
Manual quality assessment for Agent 1.
Displays selected blocks for human review.
"""
...
def manual_quality_check():
    """
    Run Agent 1 and display selected blocks for manual review.
    """
    ...
    full_tokens = loader.count_tokens(full_text)

    print(f"Full conversation: {full_tokens} tokens")
    print(f"Total messages: {len(messages)}\n")

    # Use a 50k token slice for testing (to avoid proxy context limits)
    target_tokens = 50000
    context = loader.create_slice(target_tokens=target_tokens, tolerance=0.1)
    actual_tokens = loader.count_tokens(context)
    numbered_context = add_line_numbers(context)

    print(f"Using slice: {actual_tokens} tokens (target: {target_tokens})")
    print(f"Total lines: {len(context.split(chr(10)))}\n")

    # Run Agent 1
    agent1 = Agent1Selector()
    need_to_free = min(int(actual_tokens * 0.3), 15000)  # 30% or 15k, whichever is smaller
